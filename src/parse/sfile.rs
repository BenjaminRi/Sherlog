	...
	for file in glog_files {
		...
		if name != file.name {
			if !deque.is_empty() {
				...
				let root = model::LogSource {name: name, children: {model::LogSourceContents::Entries(Vec::<model::LogEntry>::new()) } };
...
impl<'a, R: std::io::Read + std::io::Seek> ConcatZipReader<'a, R> {
	fn new(
		archive : &'a mut zip::ZipArchive<R>,
		indices : std::collections::VecDeque<usize>) -> ConcatZipReader<'a, R> {
		ConcatZipReader {archive: archive, file: None, indices : indices }
	}
}
				...
				match self.indices.pop_front() {
					Some(idx) => {
						...
						unsafe {
							...
							self.file = Some(std::mem::transmute::<zip::read::ZipFile<'_>, zip::read::ZipFile<'a>>(f));
						}
