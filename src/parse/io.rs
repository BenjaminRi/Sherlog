...
impl std::fmt::Display for LogParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
		match self {
			...
			LogParseError::UnrecognizedFileExtension(ext) => write!(f, "Unrecognized file extension: {}", ext.to_string_lossy()),
...
pub fn from_file(path: &std::path::PathBuf) -> Result<model::LogSource, LogParseError> {
	...
	if let Some(extension) = extension {
		match extension.to_string_lossy().as_ref() {
			...
			_ => Err(LogParseError::UnrecognizedFileExtension(extension.to_os_string())),
		}
	} else {
